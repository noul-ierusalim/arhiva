-- Inject one message into the SANDBOX WordPress DB (prefix `Kup_`).
-- Idempotent-ish: re-running creates a duplicate post; delete by slug first if re-testing.
-- Message: Cuvântul lui Dumnezeu la praznicul Bobotezei  (source cheie 1250, archive date 2015-01-19)

START TRANSACTION;

-- 1) Ensure the single 'Cuvântul lui Dumnezeu' category exists; capture its ids.
INSERT INTO `Kup_terms` (name, slug, term_group)
  SELECT 'Cuvântul lui Dumnezeu', 'cuvantul', 0
  WHERE NOT EXISTS (SELECT 1 FROM `Kup_terms` WHERE slug = 'cuvantul');
SET @term_id := (SELECT term_id FROM `Kup_terms` WHERE slug = 'cuvantul' LIMIT 1);

INSERT INTO `Kup_term_taxonomy` (term_id, taxonomy, description, parent, count)
  SELECT @term_id, 'category', '', 0, 0
  WHERE NOT EXISTS (SELECT 1 FROM `Kup_term_taxonomy` WHERE term_id = @term_id AND taxonomy = 'category');
SET @tt_id := (SELECT term_taxonomy_id FROM `Kup_term_taxonomy` WHERE term_id = @term_id AND taxonomy = 'category' LIMIT 1);

-- 2) The post itself. Permalink resolves to /cuvantul/2015/01/cuvantul-lui-dumnezeu-la-praznicul-bobotezei/
INSERT INTO `Kup_posts`
  (post_author, post_date, post_date_gmt, post_content, post_title, post_excerpt,
   post_status, comment_status, ping_status, post_password, post_name, to_ping, pinged,
   post_modified, post_modified_gmt, post_content_filtered, post_parent, guid,
   menu_order, post_type, post_mime_type, comment_count)
VALUES
  (1, '2015-01-19 18:00:00', '2015-01-19 16:00:00', '[et_pb_section fb_built="1" _builder_version="4.27.4" _module_preset="default" custom_margin="5px||5px||false|false" custom_padding="5px||5px||false|false" global_colors_info="{}"][et_pb_row _builder_version="4.27.4" _module_preset="default" global_colors_info="{}"][et_pb_column type="4_4" _builder_version="4.27.4" _module_preset="default" global_colors_info="{}"][et_pb_audio audio="https://noulierusalim.ro/wp-content/uploads/15.01.19.mp3" _builder_version="4.27.4" _module_preset="default" custom_margin="||||false|false" custom_padding="20px||20px||false|false" border_radii="on|7px|7px|7px|7px" box_shadow_style="preset1" global_colors_info="{}"][/et_pb_audio][et_pb_text admin_label="Cuvant" _builder_version="4.27.5" _module_preset="default" global_colors_info="{}"]În cer şi pe pământ e praznic de Bobotează, şi Mă adun cu sfinţii aici şi pun masă de cuvânt, şi pun duhul bucuriei sfinte pe masă, că Mi-a fost dor, am aşteptat cu dor această zi. Mereu aştept cu dor zilele de praznic ca să Mă adun cu sfinţii şi cu cei ce vin aici, la izvorul Meu de cuvânt ca să le dau lor din cer, ca să le dau lor. O, mare bucurie am când am pe lângă Mine pe cei ce Mă aud şi Mă simt în cuvântul Meu cel rostit ca să Mă dau lor.
...
Acum petrecerea cerească o binecuvintez şi o fac lucrătoare peste cei adunaţi la izvor, că aţi trudit, fiilor, ca să împărţiţi bucurii şi trăiri cereşti şi înţelepciune şi putere pentru iubire de Dumnezeu în toţi cei care vin cu dor la izvorul Meu slujit de voi. O, ce trebuie să facă omul mereu, mereu? Pocăinţă mereu trebuie să facă şi să aibă. Şi de ce mereu, fiilor? O, cel fără de pocăinţă mereu în el nu mai stă cuprins în duhul împărăţiei cerurilor, nu mai stă. Trebuie pocăinţă mereu, căci cine nu are mereu în el duhul pocăinţei îi uită pe cei ce n-au pocăinţă, dar cei ce Mă iubesc şi Mă urmează pentru răscumpărarea lor şi a celor ce rătăcesc de Dumnezeu, aceia fac pocăinţă şi pentru cei ce n-au decât bucuria cea de la păcat şi de la toată deşertăciunea vieţii, şi trebuie şi pentru ei chemată salvarea, căci Ioan Botezătorul striga la Iordan şi-i striga pe toţi zicându-le: <em>«Pocăiţi-vă, că s-a apropiat împărăţia cerurilor şi pregătiţi-vă pentru ea, că vine Domnul, vine plin de Duhul Sfânt şi vă botează cu El dacă vă va afla în pocăinţă!»</em>.
...
Eu sunt Cel ce sunt. Mângâiere am luat şi am dat la masa cea de azi. O, fiţi asemenea Mie, fiţi fiii mângâierii, o, fiilor. Amin, amin, amin.[/et_pb_text][/et_pb_column][/et_pb_row][/et_pb_section]', 'Cuvântul lui Dumnezeu la praznicul Bobotezei', '',
   'publish', 'closed', 'closed', '', 'cuvantul-lui-dumnezeu-la-praznicul-bobotezei', '', '',
   '2015-01-19 18:00:00', '2015-01-19 16:00:00', '', 0, '',
   0, 'post', '', 0);
SET @post_id := LAST_INSERT_ID();
UPDATE `Kup_posts` SET guid = CONCAT('https://noulierusalim.ro/?p=', @post_id) WHERE ID = @post_id;

-- 3) Divi builder meta bundle (without it Divi renders raw markup).
INSERT INTO `Kup_postmeta` (post_id, meta_key, meta_value) VALUES
  (@post_id, '_et_pb_use_builder', 'on'),
  (@post_id, '_et_builder_version', 'VB|Divi|4.27.5'),
  (@post_id, '_et_pb_built_for_post_type', 'page'),
  (@post_id, '_et_pb_page_layout', 'et_no_sidebar'),
  (@post_id, '_et_pb_show_title', 'on'),
  (@post_id, '_et_pb_side_nav', 'off'),
  (@post_id, '_et_pb_post_hide_nav', 'default'),
  (@post_id, '_et_gb_content_width', '1080'),
  (@post_id, '_thumbnail_id', '0');

-- 4) Attach to the category and bump its count.
INSERT INTO `Kup_term_relationships` (object_id, term_taxonomy_id, term_order)
  VALUES (@post_id, @tt_id, 0);
UPDATE `Kup_term_taxonomy` SET count = count + 1 WHERE term_taxonomy_id = @tt_id;
